...
/* Variables */
    ...
    u32 resumingSlice = HANTRO_FALSE;
...
/* Code */
    ...
    if (pStorage->slice->decodeInProgress && byteStrm == pStorage->prevBufPointer)
    {
        strm = pStorage->strm[0];
        *readBytes = 0;
        nalUnit = pStorage->prevNalUnit[0];
        resumingSlice = HANTRO_TRUE;
    }
    /* if previous buffer was not finished and same pointer given -> skip NAL
     * unit extraction */
    else if (pStorage->prevBufNotFinished && byteStrm == pStorage->prevBufPointer)
    {
        strm = pStorage->strm[0];
        ...
        if (!picReady)
        {
            switch (nalUnit.nalUnitType)
            {
                ...
                case NAL_CODED_SLICE:
                    ...
                    if (tmp == HANTRO_PENDING)
                    {
                        pStorage->strm[0] = strm;
                        *readBytes = 0;
                        return(H264BSD_PENDING);
                    }
                    if (tmp != HANTRO_OK)
                    {
                        EPRINT("SLICE_DATA");
    ...
    {
        DEBUG(("SLICE DATA RESUME, FIRST %d\n",
                pStorage->sliceHeader->firstMbInSlice));
        tmp = h264bsdDecodeSliceData(&strm, pStorage,
            pStorage->currImage, pStorage->sliceHeader);
        if (tmp == HANTRO_PENDING)
...
void h264bsdSetMacroblockBudget(storage_t *pStorage, u32 macroblockBudget)
{
    ...
    pStorage->macroblockBudget = macroblockBudget;
}
